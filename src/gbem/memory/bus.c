...
uint8_t mem_read(gb_t * gb, uint16_t addr) {
    ...
    else if (addr >= GB_VRAM_START && addr <= GB_VRAM_END) return               vram_read(gb, addr);
    ...
    else if (addr >= GB_OAM_START && addr <= GB_OAM_END) return                 oam_read(gb, addr);
    else if (addr >= GB_IO_REG_START && addr <= GB_IO_REG_END) return           io_read(gb, addr);
...
void mem_write(gb_t *gb, uint16_t addr, uint8_t value) {
    ...
    else if (addr >= GB_VRAM_START && addr <= GB_VRAM_END) {vram_write(gb, addr, value); return;}
    ...
    else if (addr >= GB_OAM_START && addr <= GB_OAM_END) { oam_write(gb, addr, value); return; }
